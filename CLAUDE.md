...
- Rival expuesto en la interfaz: solo `sheriff` (`DEFAULT_RIVAL`). Los demás siguen en `PERSONALITIES` para pruebas y balance. Rivales en `ai.py`: cauto (riesgo ≤20%), tahur (≤34%), loco (≤50%), sheriff (≤50%, 5 vidas, 3 balas en el tambor compartido). Cada `Personality` puede fijar `lives` y `live_rounds`; `make_duel()` construye el duelo con ellos. El turno de la IA se corta al cerrar la ronda para que el humano vea el resumen.
